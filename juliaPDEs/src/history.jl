...
#
#   runs/heatequation_1d_N50_2026-06-17T08-30-12/
...
"""
    default_run_name(problem, shape) -> String

Descriptive, sortable, timestamped folder name, e.g.
`heatequation_1d_N50_2026-06-17T08-30-12`. `shape` is the field size tuple, so
this works for every problem type regardless of how its grid is stored.
"""
function default_run_name(problem, shape)
    label = lowercase(string(nameof(typeof(problem))))
    stamp = Dates.format(now(), dateformat"yyyy-mm-ddTHH-MM-SS")
    return string(label, "_", length(shape), "d_N", join(shape, "x"), "_", stamp)
end
...
"""
    save_solution(sol::PDESolution, dir = joinpath("runs", default_run_name(...)))

Save one solution as a one-frame run folder (`meta.json` + `step_00000.json`).
Returns the folder path.
"""
function save_solution(sol::PDESolution,
                       dir::AbstractString = joinpath("runs",
                           default_run_name(getfield(sol, :problem), size(sol.u))))
